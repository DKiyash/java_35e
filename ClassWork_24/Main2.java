...
//        Arrays.stream(strings).map(s -> s.replaceAll("\\W+", " "))
...
//                .forEach(System.out::println);
        ...
        Arrays.stream(words).map(s -> s.replaceAll("\\W+", " "))
                .filter(s -> s.matches("[A-Z]\\w*"))
                .filter(s -> s.length() > 3)
...
//Модифицировать стрим, заменив последний map на 2 вызова map,
//чтобы получить тот же самый результат. За 2 вызова можно использовать
// только 1 substring. (нужно использовать в одном месте регулярку)